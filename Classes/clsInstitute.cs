	...
	public class clsInstitute
	{
		...
		public static string InstituteName(string pkUniID,string pkInstID)
		{
			...
			try
			{
				...
				DataTable DT =oDB.getparamdataset("ID_InstituteName",oHs).Tables[0]; 
				if(DT.Rows.Count>0)
				{
		...
		public static DataTable InstituteSearch(string Uni_ID,string InstTy_ID,string Inst_Name,string Country_ID,string State_ID,string District_ID,string Tehsil_ID)
		{
			DBObjectPool Pool=null;
			DBObject oDB=null;
			try
			{
				...
				DataTable DT =oDB.getparamdataset("ID_SearchInstitute",oHs).Tables[0]; 
				return DT;
			}
			finally
			{
