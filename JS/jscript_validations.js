...
function validateMe(Arr, DialogHeight)
{
	...
	for (k = 0; k < Arr.length; k++)
	{
		...
		if(elementType == "CheckSelected") // Added By Shrikant
		{
		    var isChecked = false;
		    myElement1 = myElement.split("|");
	        for(var i = 0; i < myElement1.length; i++)
	        {
	            if(document.getElementById(myElement1[i]).checked)
	            {
	                isChecked = true;
	                break;
	            }
		...
		else if(elementType=="CompareDates")
		{
			var myElement1;
			var DateOne;
			var DateTwo;
...
//--------------------------
		...
		else if(elementType=="date")
		{
		    ...
		    if(whatToDo == "notGreaterThanToday")
		    {
			...
			else if(myElement.value.length !=0)
			{
				/*
				var invalidChars = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ\{\}\'\:\)\(\,\=\+\_\~\`\!\@\#\$\%\^\&\*\;\>\<\]\[ ";
...
//---- added on 09-Dec-2004 by Ajay (approved by khalique Sir)
			...
			if(whatToDo=="Decimal/Zero")//Added by Amit
			{
			    ...
			    if (myValue.length != 0)				
			    {
			        ...
			        if(myValue.charAt(0)=='.')//for values like .99 (digit prefix by .)
			        {
			            myValue=0+myValue;
			            var str=/^(-)?(\d)+(\.(\d)+)?$/.test(myValue);			          
	                    if (!str)
	                    {
	                        message = message + (++count) + ". " + myMessage + "\n";	
					        gotErr="Y";	
			        ...
			        else//for values like starts with digit then one . between digit
			        {	        
			            var str=/^(-)?(\d)+(\.(\d)+)?$/.test(myValue);
			            //var ptrn = new RegExp(str);	
			...
			if(whatToDo=="DecimalNonZero")//Added by Amit
			{
			   
			  
			    if (myValue.length != 0)				
			    {
			        if(myValue.charAt(0)=='0')//for values like .99 (digit prefix by .)
			        {
			            myValue=myValue.replace(myValue.charAt(0),'');
