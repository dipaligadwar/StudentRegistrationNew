<%@ Control Language="c#" AutoEventWireup="True" Codebehind="Elg_StudentBulklisting.ascx.cs" Inherits="StudentRegistration.Eligibility.WebCtrl.Elg_StudentBulklisting" TargetSchema="http://schemas.microsoft.com/intellisense/ie5"%>
...
<script lang="javascript">
		/*function to check and uncheck all rows */
		function CheckAll(PageSize,DataGridName,ChkName1,ChkName2)
		{
	//alert("hi");
	//alert(""+PageSize+DataGridName+ChkName1+ChkName2)
			var PS=parseInt(PageSize)+3;
			var id1=""+DataGridName+"__ctl";
			var id2= "_"+ChkName2;
			var str=""+id1+""+2+"_"+ChkName1;
			var MainChk=document.getElementById("Elg_StudentBulklisting1_"+id1+""+2+"_"+ChkName1); 
			//alert(MainChk);
			//alert(""+id1+""+2+"_"+ChkName1);
			var start = 3;
			if(MainChk!=null)
			{
		//	alert(MainChk.checked);
			if(MainChk.checked==true)
			{
				for (var i=start;i<PS;i++)
				{
					var chk=document.getElementById("Elg_StudentBulklisting1_"+id1+""+i+id2); 
					if(chk!=null)
						chk.checked=true; 
					else
						break;
		...
		/* function called while checking on each select checkbox.*/
		function Checkone(PageSize,DataGridName,ChkName1,ChkName2)
		{
		alert("hi");
		alert(""+PageSize+DataGridName+ChkName1+ChkName2)
		var PS=parseInt(PageSize)+3;
...
<script language="javascript">
		var uniid;
		uniid = <%=System.Configuration.ConfigurationSettings.AppSettings["UniversityID"]%>
				...
				<table id="tblDGElgRegular" style="DISPLAY: none" width="100%" runat="server">
					<TR>
						<td><asp:datagrid id="dgElgRegular" runat="server" Width="100%" BorderWidth="1px" BorderStyle="Solid"
								AllowPaging="True" BorderColor="#336699" AutoGenerateColumns="False" onselectedindexchanged="dgElgRegular_SelectedIndexChanged">
								...
								<Columns>
									...
									<asp:TemplateColumn HeaderText="chkboxtemplateColumn">
										<HeaderTemplate>
											<asp:CheckBox id="SelectAll" runat="server" Text="SelectAll"></asp:CheckBox>
										</HeaderTemplate>
										<ItemTemplate>
											<asp:CheckBox id="Select" runat="server" Text="Select"></asp:CheckBox>
										</ItemTemplate>
										<HeaderStyle CssClass="gridHeader" HorizontalAlign=Center />
									</asp:TemplateColumn>
								</Columns>
					...
					<tr>
						<td align="center" colSpan="3"><asp:button id="btndecision" CssClass="butSubmit" Runat="server" Text="Submit Decision" onclick="btndecision_Click"></asp:button></td>
					</tr>
				</table>
			</td>
		</TR>
