...
//--------------------------
		...
		else if(elementType=="text") //Validations if form element type is text box
		{
			...
			if(whatToDo == "AlphaNumericOnly") // validates to disallow characters
			{
				var invalidChars = "\{\}\/\'\:\)\(\,\-\=\+\_\~\`\"\!\@\#\$\%\^\&\*\;\>\<\]\[\?";
			...
			if(whatToDo == "AlphaNumericOnly/Empty") // validates to disallow characters
			{
			...
			if(whatToDo == "NumericOnlyRoleMembres") // Added by Gurpreet to accept "", -, NA and Numbers
			{

				var invalidChars = "bcdefghijklmopqrstuvwxyzBCDEFGHIJKLMOPQRSTUVWXYZ\{\}\/\'\:\)\(\,\=\+\_\~\`\!\@\#\$\%\^\&\*\;\>\<\]\[ ";
...
//--------------------------
			...
			{
				var invalidChars = "\{\}\/\'\:\)\(\,\-\=\+\_\~\`\!\@\#\$\%\^\&\*\;\>\<\.\]\[\"";   //Added on 06/03/06 By Jyotsna
				for (l=0; l < invalidChars.length; l++)
				{
					if(myValue.indexOf(invalidChars.charAt(l)) > -1)
					{
